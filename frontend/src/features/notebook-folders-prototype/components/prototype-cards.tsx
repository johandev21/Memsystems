      ...
      <ContextMenuTrigger
        render={
          <div
            ...
            {...listeners}
            className={cn(
              "prototype-folder-card",
              isDragging && "prototype-notebook-card--dragging",
            )}
            ...
            tabIndex={0}
            aria-label={`${folder.name}, ${notebooks.length} notebooks`}
            onKeyDown={(event) => {
...
function FolderArtwork({
  ...
  const titleSlot = (
    <InlineEditableText
      ...
      tooltip={title}
      className="prototype-artwork__title-editor"
      inputClassName="prototype-artwork__title-input"
    >
      <span>{title}</span>
    </InlineEditableText>
  );
  ...
  return (
    <span className="prototype-artwork">
      {notebooks.length === 0 ? (
...
export function NotebookCard({
  ...
  const titleSlot = (
    <InlineEditableText
      ...
      className="prototype-artwork__title-editor"
      inputClassName="prototype-notebook-title-input"
    >
    ...
    <ContextMenu>
      ...
      <ContextMenuContent>
        ...
        {folders.map((folder) => (
          ...
          >
            <FolderInput />
            Move To {folderPaths?.get(folder.id) ?? folder.name}
          </ContextMenuItem>
        ))}
      </ContextMenuContent>
    </ContextMenu>
...
export function NotebookPreview({ notebook }: NotebookPreviewProps) {
  return (
    <div className="prototype-notebook-preview" aria-label={`Moving ${notebook.title}`}>
...
export function FolderPreview({ folder, notebooks }: FolderPreviewProps) {
  return (
    <div className="prototype-folder-card" aria-label={`Moving ${folder.name}`}>
      <FolderArtwork
        title={folder.name}
        notebooks={notebooks}
        onRename={() => {}}
