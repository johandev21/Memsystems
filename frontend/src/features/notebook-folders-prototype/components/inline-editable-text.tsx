...
export function InlineEditableText({
  ...
  if (editing) {
    ...
    const shared = {
      ...
      maxLength,
      className: cn("prototype-inline-editable__input", inputClassName),
      "aria-label": ariaLabel,
    };
    return multiline ? <textarea {...shared} ref={assignRef} rows={3} /> : autoSize ? (
      <span className="prototype-inline-editable__sizer">
  ...
  const button = (
    <button
      ref={buttonRef}
      type="button"
      className={cn("prototype-inline-editable", className)}
      onClick={(event) => {
        event.stopPropagation();
        setEditing(true);
      }}
