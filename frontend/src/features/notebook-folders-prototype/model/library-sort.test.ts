...
function folder(id: string, name: string, dates: Partial<Folder> = {}): LibraryItem {
  return {
    kind: "folder",
    folder: {
      id,
...
function notebook(id: string, title: string, dates: Partial<Notebook> = {}): LibraryItem {
  return {
    kind: "notebook",
    notebook: {
      id,
...
describe("sortLibraryItems", () => {
  it("sorts names case-insensitively and naturally", () => {
    const items = [notebook("b", "b Notebook"), notebook("c", "Notebook 10"), notebook("a", "Notebook 2")];
    expect(labels(sortLibraryItems(items, "name"))).toEqual(["b Notebook", "Notebook 2", "Notebook 10"]);
  });

  it("interleaves folders and notebooks by name", () => {
    const items = [folder("f1", "Classical Philosophy"), notebook("n1", "Aristotle"), folder("f2", "Modern Philosophy"), notebook("n2", "David Hume")];
    expect(labels(sortLibraryItems(items, "name"))).toEqual(["Aristotle", "Classical Philosophy", "David Hume", "Modern Philosophy"]);
  });
  ...
  it("falls back to name, then folders before notebooks, on date ties", () => {
    ...
    expect(sortLibraryItems(items, "updatedAt").map((item) => `${item.kind}:${labels([item])[0]}`)).toEqual(["folder:Alpha", "notebook:Alpha", "folder:Beta"]);
  });
...
describe("toLibraryItems", () => {
  it("wraps folders and notebooks with their kind", () => {
    const items = toLibraryItems(
      [{ id: "f", name: "Folder", createdAt: "2026-01-01T00:00:00.000Z", updatedAt: "2026-01-01T00:00:00.000Z" }],
      [],
    );
