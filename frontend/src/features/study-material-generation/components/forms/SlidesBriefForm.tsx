...
type SlidesThemeOption = "dark" | "light" | "accent" | "editorial" | "academic" | "technical" | "warm";
type DetailLevel = "basic" | "detailed";

const SLIDE_PRESETS = [5, 8, 10, 12];
const MAX_SLIDE_COUNT = 20;

function normalizeTheme(theme: SlidesThemeOption | undefined): Exclude<SlidesThemeOption, "accent"> {
  if (theme === "accent") return "warm";
  return theme ?? "dark";
}

const THEME_OPTIONS = [
  { id: "dark" as const, title: "Dark", desc: "Slate deck, cyan accent", swatch: { bg: "#0F172A", surface: "#1E293B", accent: "#38BDF8", text: "#F8FAFC" } },
  { id: "light" as const, title: "Light", desc: "Bright, print-friendly", swatch: { bg: "#FFFFFF", surface: "#F1F5F9", accent: "#0EA5E9", text: "#0F172A" } },
  { id: "editorial" as const, title: "Editorial", desc: "Warm paper, serif voice", swatch: { bg: "#FDFBF7", surface: "#F5EFE6", accent: "#B45309", text: "#1C1917" } },
  { id: "academic" as const, title: "Academic", desc: "Classic lecture style", swatch: { bg: "#FFFFFF", surface: "#EFF6FF", accent: "#1D4ED8", text: "#111827" } },
  { id: "technical" as const, title: "Technical", desc: "Deep navy, neon detail", swatch: { bg: "#020617", surface: "#0F172A", accent: "#22D3EE", text: "#E2E8F0" } },
  { id: "warm" as const, title: "Warm", desc: "Bold ember energy", swatch: { bg: "#FFF7ED", surface: "#FFEDD5", accent: "#EA580C", text: "#431407" } },
] as const;
...
export function SlidesBriefForm({
  ...
  return (
    <div className="flex w-full flex-col gap-4 font-sans text-text-tertiary">
      ...
      {step === 1 ? (
        <div className="flex flex-col gap-4 min-h-[380px] justify-between animate-in fade-in slide-in-from-right-2 duration-150">
                ...
                {THEME_OPTIONS.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      aria-pressed={selected}
                      aria-label={`${opt.title} theme: ${opt.desc}`}
                      onClick={() => setTheme(opt.id)}
                      className={cn(
                        optionRowClass(selected),
                        "p-2.5 flex flex-col gap-2 cursor-pointer text-left",
                      )}
                    >
                      <span
                        aria-hidden="true"
                        className="relative block aspect-video w-full overflow-hidden rounded-lg border border-surface-border-subtle"
                      ...
                      >
                        ...
                        />
                        <span
                          className="absolute left-1.5 top-2.5 block h-1.5 w-3/5 rounded-full"
                          style={{ backgroundColor: opt.swatch.text }}
                        />
                        <span
                          className="absolute left-1.5 top-[18px] block h-1 w-2/5 rounded-full opacity-70"
                          style={{ backgroundColor: opt.swatch.text }}
                        />
                        <span className="absolute bottom-1.5 left-1.5 flex gap-1">
                          <span
                            className="block h-2.5 w-2.5 rounded"
                            style={{ backgroundColor: opt.swatch.surface, border: `1px solid ${opt.swatch.accent}` }}
                          />
                          <span
