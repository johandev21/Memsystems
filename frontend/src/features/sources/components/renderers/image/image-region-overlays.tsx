        ...
        return (
          <button
            ...
            className={cn(
              ...
              "left-(--region-left) top-(--region-top) w-(--region-width) h-(--region-height)",
            )}
            style={
              {
                "--region-left": `${Math.max(0, Math.min(100, region.x * 100))}%`,
                "--region-top": `${Math.max(0, Math.min(100, region.y * 100))}%`,
                "--region-width": `${Math.max(0, Math.min(100, region.width * 100))}%`,
                "--region-height": `${Math.max(0, Math.min(100, region.height * 100))}%`,
          ...
          >
            <span
              className={cn(
                "absolute -top-3.5 -left-0.5 rounded px-1 py-0 text-xs font-bold shadow-xs whitespace-nowrap",
                isActive || isCitationTarget
