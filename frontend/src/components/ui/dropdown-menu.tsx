...
function DropdownMenuContent({
  ...
  const contextLayer = useOverlayLayer();
  const layerClass = layerClassName ?? contextLayer ?? "z-popover";
  return (
    <MenuPrimitive.Portal>
      <MenuPrimitive.Positioner
        className={cn("isolate outline-none", layerClass)}
        align={align}
...
function DropdownMenuItem({
  ...
  return (
    <MenuPrimitive.Item
      ...
      className={cn(
        "group/dropdown-menu-item relative flex min-h-7 cursor-default items-center gap-2 rounded-xl px-2 py-1.5 text-sm outline-hidden select-none whitespace-nowrap focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground data-inset:pl-7 data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:bg-destructive/10 data-[variant=destructive]:focus:text-destructive dark:data-[variant=destructive]:focus:bg-destructive/20 data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 data-[variant=destructive]:*:[svg]:text-destructive",
        className,
      )}
      {...props}
    />
