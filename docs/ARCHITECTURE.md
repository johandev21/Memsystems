...
| `retrieval_traces` | One retrieval trace per Chat turn and Study Material Generation: query, query-understanding decision and queries, ranked candidates per leg, fusion knobs and fused order, reranker model and scores, chosen chunks, thresholds, latency, and token cost. Correlation ids are not foreign keys so a trace survives a turn that failed before its row was written. |
