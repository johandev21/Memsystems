...
### Key patterns
- **API client**: `src/lib/api-client/` — `createQueryOptions` / `apiPost` / `apiDelete` typed wrappers with automatic 401 handling.
- **Auth**: `src/shared/auth/` — Clerk auth wrapper (`useAuth`, `AuthState`).
...
- **Auth**: `src/modules/auth/` — Clerk request authentication seam (`AuthGuard`, `RequestAuthenticator`, `ClerkRequestAuthenticator`).
...
- **Database schema**: Defined in `src/database/schema.ts` without local user tables; domain tables store Clerk IDs directly as indexed `user_id` text columns.
...
- `APP_ORIGIN` is the single browser-facing origin used by CORS and signed local-storage URLs.
