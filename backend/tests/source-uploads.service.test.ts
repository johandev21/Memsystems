...
describe('SourceUploadsService', () => {
  it('streams a local upload and finalizes only the issued target', async () => {
    ...
    const target = await service.createTarget(user.id, notebook.id, {
      filename: 'notes.txt',
    ...
    const source = await service.finalize(
      user.id,
      notebook.id,
      target.uploadId,
    );
  ...
  it('rejects a target used by another user before reading the body', async () => {
    const service = makeService(await newDirectory());
    const owner = await seedUser();
    const notebook = await seedNotebook(owner.id);
    const target = await service.createTarget(owner.id, notebook.id, {
      filename: 'notes.txt',
      contentType: 'text/plain',
      size: 1,
    });
    ...
    await expect(
      service.uploadLocal(
        'intruder',
        target.uploadId,
        requestFrom(Buffer.from('x')) as any,
      ),
  ...
  it('can finalize a completed upload after the service is recreated', async () => {
    ...
    await expect(
      recreated.finalize(user.id, notebook.id, target.uploadId),
  ...
  it('allows only one concurrent finalizer to consume an intent', async () => {
    ...
    const results = await Promise.allSettled([
      service.finalize(user.id, notebook.id, target.uploadId),
      service.finalize(user.id, notebook.id, target.uploadId),
    ]);
    expect(
  ...
  it('enforces 20 MB MAX_IMAGE_BYTES limit for images while allowing up to 50 MB for documents', async () => {
    ...
    await expect(
      service.createTarget(user.id, notebook.id, {
        filename: 'large.png',
    ...
    const imageTarget = await service.createTarget(user.id, notebook.id, {
      filename: 'valid.png',
      contentType: 'image/png',
      size: 20 * 1024 * 1024,
    });
    ...
    const docTarget = await service.createTarget(user.id, notebook.id, {
      filename: 'large.pdf',
  ...
  it('enforces 500 MB MAX_AUDIO_BYTES limit for audio files', async () => {
    ...
    const audioTarget = await service.createTarget(user.id, notebook.id, {
